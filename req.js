...
$.get("http://insight.chaucha.cl/api/addr/cZJXM2yPqjbinTZ48fV8tjGqjxAePJeWuQ", function(tx) {
	var info = [];
	$.each(tx['transactions'], function( index, txin ) {
		$.get("http://insight.chaucha.cl/api/tx/" + txin, function(op_return) {
			$.each(op_return['vout'], function ( index , op ) {
				if(op['scriptPubKey']['asm'].indexOf('RETURN') > 0) {
					...
					output = '<a href="http://insight.chaucha.cl/tx/' + txin + '" target="_blank">' + e2t(timestamp) +
					'</a>: ' + msg_hex + ' (' + msg + ')<br>';
